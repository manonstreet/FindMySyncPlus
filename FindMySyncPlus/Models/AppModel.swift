...
final class AppModel: NSObject, ObservableObject {
    ...
    func invalidateFriendDecryptorKey() {
        ...
        Task { await friendDecryptor.invalidateKey() }
    }
    ...
    private func buildPlanAndLog(
        ...
        // prsId is "owner" for the user's own devices, or a base64 DSID for family members.
        // Friends whose handleServerIdentifier matches a family DSID are duplicates.
        ...
        for f in friendEntries {
            ...
            // DSID-based dedup: skip friends whose handleServerIdentifier matches
            // a family member's prsId in Devices.data (they already have device entries)
            ...
            if let rec = aliasByUUIDLocal[friendID] {
                // Friend is already assigned to an alias (by handleServerIdentifier)
                if !f.name.isEmpty {
    ...
    private func _runOnceAsync(kind: RunKind, dryRun: Bool) async {
        ...
        if settings.enableFriends {
            await friendDecryptor.ensureKey(logger: logger)
        ...
        if hasFriendSource {
            switch await friendDecryptor.readFriendLocations(logger: logger) {
        ...
        if !friendEntries.isEmpty {
            if let fmfNames = await decryptor.readFMFContactNames(logger: logger) {
